...
const HistorialTutorias = () => {
  ...
  const fetchTutorias = async () => {
    try {
      const response = await fetch('http://192.168.0.46:3001/api/tutoring/tutorias');
  ...
  const handleDeleteClick = async (tutoriaId, onTutoriaDelete = () => {}) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`http://192.168.0.46:3001/api/tutoring/tutorias/${tutoriaId}`, {
          method: 'DELETE',
        });
